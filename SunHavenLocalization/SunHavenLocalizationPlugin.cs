...
namespace SunHavenLocalization
{
    ...
    public class SunHavenLocalizationPlugin : BaseUnityPlugin
    {
        ...
        public static HashSet<string> SuccKeys = new HashSet<string>();
        public static HashSet<string> FailKeys = new HashSet<string>();
        ...
        private static void PatchOptionalGameMethods(Harmony harmony)
        {
            ...
            PatchOptionalDialogueMethod(harmony);
            PatchOptionalBuffMethods(harmony);
        }

        private static void PatchOptionalDialogueMethod(Harmony harmony)
        {
            var dialogueHelperType = AccessTools.TypeByName("Wish.DialogueHelper");
            var replace = dialogueHelperType == null
                ? null
                : AccessTools.Method(dialogueHelperType, "Replace", new[] { typeof(string) });

            if (replace == null)
            {
                LogWarning("Wish.DialogueHelper.Replace not found; hardcoded dialogue patch skipped.");
                return;
            }
            ...
            try
            {
                harmony.Patch(
                    replace,
                    postfix: new HarmonyMethod(typeof(SunHavenLocalizationPlugin), nameof(DialogueHelper_Replace_Postfix)));
                LogInfo("Patched Wish.DialogueHelper.Replace.");
            }
            catch (Exception ex)
            {
                LogError($"Failed to patch Wish.DialogueHelper.Replace: {ex}");
            }
        }

        private static void PatchOptionalBuffMethods(Harmony harmony)
        {
            string[] buffTypeNames = { "Wish.StatBuff", "Wish.HourlyBuff" };
            foreach (string buffTypeName in buffTypeNames)
            {
                var buffType = AccessTools.TypeByName(buffTypeName);
                var startBuff = buffType == null ? null : AccessTools.Method(buffType, "StartBuff");
                if (startBuff == null)
                {
                    LogWarning($"{buffTypeName}.StartBuff not found; hardcoded buff text patch skipped.");
                    continue;
                }
                ...
                try
                {
                    harmony.Patch(
                        startBuff,
                        prefix: new HarmonyMethod(typeof(SunHavenLocalizationPlugin), nameof(Buff_StartBuff_Prefix)));
                    LogInfo($"Patched {buffTypeName}.StartBuff.");
                }
                catch (Exception ex)
                {
                    LogError($"Failed to patch {buffTypeName}.StartBuff: {ex}");
                }
            }
        }
        ...
        private static string FixKnownHardcodedText(string text)
        {
            ...
            if (ExactHardcodedTextLocalizationKeys.TryGetValue(trimmed, out var localizationKey) &&
                TryGetLoadedTranslation(localizationKey, out exactTranslation))
            {
                return PreserveOuterWhitespace(text, exactTranslation);
            }
        ...
        private static string FixDynamicEnglishFragments(string text)
        {
            ...
            result = FixDonationBoxText(result);
            result = FixKnownLocalizedFragments(result);
        ...
        private static string FixDonationBoxText(string text)
        {
            if (string.IsNullOrEmpty(text) ||
                (!text.Contains("donate mana orbs") &&
                 !(text.Contains("Donate ") && text.Contains("mana orb"))))
            {
                return text;
            }

            string result = text;
            if (text.Contains("donate mana orbs") &&
                TryGetLoadedTranslation("Hardcoded.DonationBox.Prompt", out var promptTemplate))
            {
                result = DonationPromptRegex.Replace(
                    result,
                    match => promptTemplate.Replace("1111", match.Groups[1].Value),
                    1);
            }

            if (result.Contains("Donate ") &&
                result.Contains("mana orb") &&
                TryGetLoadedTranslation("Hardcoded.DonationBox.Option", out var optionTemplate))
            {
                result = DonationOptionRegex.Replace(
                    result,
                    match => optionTemplate.Replace("1111", match.Groups[1].Value),
                    1);
            }
        ...
        private static string FixKnownLocalizedFragments(string text)
        {
            const string returnToAmanda = "Return to Amanda at the library";
            if (text.Contains(returnToAmanda) &&
                TryGetLoadedTranslation("Quest.ReturnToAmanda", out var translation))
            {
                return text.Replace(returnToAmanda, translation);
            }
        ...
        public static void DialogueHelper_Replace_Postfix(ref string __result)
        {
            try
            {
                if (LoadedLocStorage == null || string.IsNullOrEmpty(__result))
                    return;

                if (!ShouldApplyChineseLocalization())
                    return;

                if (!LooksLikeKnownHardcodedText(__result))
                    return;

                __result = FixKnownHardcodedText(__result);
            }
            catch (Exception ex)
            {
                LogError($"DialogueHelper.Replace postfix exception: {ex}");
            }
        }

        public static void Buff_StartBuff_Prefix(ref string ___buffName, ref string ___buffDescription)
        {
            try
            {
                if (LoadedLocStorage == null)
                ...
                if (LooksLikeKnownHardcodedText(___buffName))
                    ___buffName = FixKnownHardcodedText(___buffName);

                if (LooksLikeKnownHardcodedText(___buffDescription))
                    ___buffDescription = FixKnownHardcodedText(___buffDescription);
            }
            catch (Exception ex)
            {
                LogError($"Buff.StartBuff prefix exception: {ex}");
            }
        }
        ...
        private static bool LooksLikeKnownHardcodedText(string text)
        {
            ...
            if (text.Contains("point(s)") ||
                ...
                text.Contains("donate mana orbs") ||
                text.Contains("mana orb") ||
                text.Contains("Return to Amanda at the library") ||
                text.Contains("Loading Scenes") ||
            ...
            return ExactHardcodedTextReplacements.ContainsKey(trimmed) ||
                   ExactHardcodedTextLocalizationKeys.ContainsKey(trimmed);
        }

        private static readonly System.Text.RegularExpressions.Regex DonationPromptRegex =
            new System.Text.RegularExpressions.Regex(
                @"Would you like to donate mana orbs\s*<sprite=""orb_icon""\s*index\s*=\s*0>\s*to the town of Nel'Vari\?\s*\((\d+) donated so far\)",
                System.Text.RegularExpressions.RegexOptions.Compiled);

        private static readonly System.Text.RegularExpressions.Regex DonationOptionRegex =
            new System.Text.RegularExpressions.Regex(
                @"Donate\s+(\d+)\s+mana orbs?\s*<sprite=""orb_icon""\s*index\s*=\s*0>",
                System.Text.RegularExpressions.RegexOptions.Compiled);
        ...
        private static readonly Dictionary<string, string> ExactHardcodedTextLocalizationKeys = new Dictionary<string, string>
        {
            // 游戏逻辑直接写入 DialogueNode / Popup / TMP，不经过 I2。
            { "Oh finally! I've been looking for Faeyon, but I couldn't hear her anywhere! Well here's the back scratcher she needed. Thanks for delivering it!", "Quest.YouScratchMyBack.DialogueOverride" },
            { "Thanks.", "Thanks" },
            { "See ya!", "SeeYaExcited" },
            { "I'm in need of a new makeover, but then again, so does this salon.", "Hardcoded.RepairSign.BarberSalon" },
            { "Donate", "Hardcoded.DonationBox.Interaction" },
            { "Increases Win Area and Sweet Spot during fishing minigame by 99% for 1 hour.", "LegendaryFishBait.Buff.Description" },
            { "How do I do that?", "HowDoIDoThat" },
            { "Return to Amanda at the library", "Quest.ReturnToAmanda" },
            { "You can't upgrade a house with animals inside!", "Hardcoded.PlayerHouse.AnimalsInside" },
            { "Don't mention it.", "DontMentionItPeriod" },
            { "Loading Scenes", "Hardcoded.LoadingScenes" },
            { "Loading World", "LoadingWorld" },
            { "Loading World from Host", "LoadingWorldFromHost" },
        };
